...
extern struct znFAT_Init_Arg * data pArg; //初始化参数结构体指针，用以指向某一存储设备的初始化参数结构体，由外部提供
//extern UINT8 data Dev_No;//设备号，外部变量
...
UINT8 znFAT_XCopy_File(struct znFAT_Init_Arg *pArg1,struct znFAT_Init_Arg *pArg2,CONST INT8 *sfilename,CONST INT8 *tfilename,UINT8 *file_buf,UINT32 buf_size,unsigned char *pt); //文件拷贝，支持多设备中文件互拷
