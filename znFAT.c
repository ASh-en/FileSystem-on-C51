...
INT8 data temp_dir_name[13]; 
UINT32 data temp_dir_cluster;
UINT32 data temp_last_cluster;
...
void znFAT_Empty_Cluster(UINT32 Cluster)
{
 ...
 {
  SD_Write_Sector(iSec,znFAT_Buffer);
 }
}
...
{
 SD_Read_Sector(1+pArg->BPB_Sector_No,znFAT_Buffer);
...
void znFAT_Init(void)
{
 ...
 SD_Read_Sector(pArg->BPB_Sector_No,znFAT_Buffer);
...
void znFAT_Update_FSInfo_Free_Clu(UINT32 PlusOrMinus)
{
 ...
 SD_Write_Sector(1+pArg->BPB_Sector_No,znFAT_Buffer);   
}
...
UINT32 znFAT_GetNextCluster(UINT32 LastCluster)
{
 ...
 SD_Read_Sector(temp,znFAT_Buffer);
...
UINT32 znFAT_Enter_Dir(CONST INT8 *path)
{
 ...
 UINT8 data i=1,counter=0,flag=0;
  ...
  while(path[i]!=0)
  {
   if(path[i]=='\\')
   {
	...
	do
	{
	 ...
	 {
	  SD_Read_Sector(iSec,znFAT_Buffer);
...
UINT8 znFAT_Open_File(struct FileInfoStruct *pfi,CONST INT8 *filepath,UINT32 item,UINT8 is_file)
{
 ...
 UINT8 data flag=0,index=0,i=0,h=0;
...
UINT32 znFAT_Read_File(struct FileInfoStruct *pfi,UINT32 offset,UINT32 len,UINT8 *pbuf)
{
 ...
 if(offset<pfi->FileSize)
 {
  ...
  SD_Read_Sector(pfi->FileCurSector,znFAT_Buffer);
...
{
 SD_Read_Sector(pArg->FirstFATSector+(cluster*4/pArg->BytesPerSector),znFAT_Buffer);
 ...
 SD_Write_Sector(pArg->FirstFATSector+(cluster*4/pArg->BytesPerSector),znFAT_Buffer);

 SD_Read_Sector(pArg->FirstFATSector+pArg->FATsectors+(cluster*4/pArg->BytesPerSector),znFAT_Buffer);
 znFAT_Buffer[((cluster*4)%pArg->BytesPerSector)+0]=dat&0x000000ff;
 znFAT_Buffer[((cluster*4)%pArg->BytesPerSector)+1]=(dat&0x0000ff00)>>8;
 znFAT_Buffer[((cluster*4)%pArg->BytesPerSector)+2]=(dat&0x00ff0000)>>16;
 znFAT_Buffer[((cluster*4)%pArg->BytesPerSector)+3]=(dat&0xff000000)>>24;
 SD_Write_Sector(pArg->FirstFATSector+pArg->FATsectors+(cluster*4/pArg->BytesPerSector),znFAT_Buffer); 
}
...
UINT8 znFAT_Create_Rec(struct FileInfoStruct *pfi,UINT32 cluster,INT8 *name,UINT8 is_dir,UINT8 *ptd)
{
 ...
 if(!flag)
 {
  ...
  temp_Rec=0;
  SD_Read_Sector(temp_sec,znFAT_Buffer);
  if(is_dir)
  {
   if(!(new_clu=znFAT_Find_Free_Clust(0))) return -1;
   znFAT_Modify_FAT(new_clu,0x0fffffff);
  ...
  SD_Write_Sector(temp_sec,znFAT_Buffer);
 }
 if(is_dir)
 {
...
UINT32 znFAT_Add_Dat(struct FileInfoStruct *pfi,UINT32 len,UINT8 *pbuf)
{
 ...
 if(len>0)
 {
  SD_Read_Sector(pfi->Rec_Sec,znFAT_Buffer);
  ...
  {   
   ...
   SD_Write_Sector(pfi->Rec_Sec,znFAT_Buffer);
  }
  else
  {
  ...
  SD_Write_Sector(pfi->FileCurSector,znFAT_Buffer); //数据接缝  
...
UINT8 znFAT_Del_File(CONST INT8 *filepath)
{
 ...
 SD_Read_Sector(fi.Rec_Sec,znFAT_Buffer);
 *(znFAT_Buffer+fi.nRec)=0xe5;
 SD_Write_Sector(fi.Rec_Sec,znFAT_Buffer);
